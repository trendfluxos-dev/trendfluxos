...
DROP POLICY IF EXISTS "Public can submit a growth lead" ON public.growth_leads;

CREATE POLICY "Public can submit a growth lead"
ON public.growth_leads
FOR INSERT
WITH CHECK (
  -- Content validation (both branches)
  ...
  AND (
    array_length(services, 1) IS NULL
    OR (
      array_length(services, 1) <= 25
      AND source = ANY (ARRAY['growth-os-landing','growth-os-hub','trendflux-contact','fluxbeam-contact'])
    )
  )
  -- Lifecycle/CRM fields must be neutral defaults regardless of branch
  AND stage = 'new'
