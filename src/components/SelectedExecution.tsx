      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="max-w-3xl">
          ...
          <h2
            ...
            className="mt-6 font-display text-3xl md:text-5xl leading-[1.1] font-semibold text-white"
          >
          ...
          <p className="mt-8 inline-flex items-center gap-3 text-[11px] uppercase tracking-[0.3em] text-white/40">
        ...
        {/* Project grid */}
        <div className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-px bg-white/[0.06] border border-white/[0.06]">
          {PROJECTS.map((p) => {
            const isExternal = p.external || p.href.startsWith("http");
            return (
              <article
                key={p.index}
                className="group flex flex-col bg-[#0a0a0a] transition-colors hover:bg-[#111]"
              >
                <button
                  type="button"
                  onClick={() => setActive(p)}
                ...
                <div className="flex flex-1 flex-col gap-6 p-8 md:p-10">
                  <header>
                    <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.35em] text-white/40">
                      <span className="font-mono text-white/70">{p.index}</span>
                      <span aria-hidden className="h-px w-6 bg-white/15" />
                      <span>{p.category}</span>
                    </div>
                    <h3 className="mt-5 font-display text-xl md:text-2xl leading-snug text-white">
                      {p.title}
                    </h3>
                    <p className="mt-4 text-sm leading-relaxed text-white/60">{p.summary}</p>
                  </header>
                  ...
                  <div>
                    ...
                    <ul className="mt-3 grid gap-1.5 text-sm text-white/75">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-2 pt-2 border-t border-white/[0.06]">
                    {p.stack.map((s) => (
                      <span
                        key={s}
                  ...
                  <div className="mt-auto flex items-center justify-between gap-4 pt-2">
                    <button
                      type="button"
                      onClick={() => setActive(p)}
                      className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.35em] text-white/70 hover:text-[hsl(0_72%_55%)] transition-colors"
                    >
                      Preview project
                    ...
                    {isExternal ? (
                      <a
                        ...
                        className="text-[10px] uppercase tracking-[0.3em] text-white/35 hover:text-white/70 transition-colors"
                      >
                        Open ↗
                      </a>
                    ) : (
...
function PreviewModal({
  ...
  return (
    ...
    >
      <div
        className="relative flex h-full max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden bg-[#0a0a0a] border border-white/[0.08] shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <header className="flex items-center justify-between gap-4 border-b border-white/[0.06] px-6 py-4 md:px-8">
          <div className="min-w-0">
            <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.35em] text-white/40">
              <span className="font-mono text-[hsl(0_72%_55%)]">{project.index}</span>
              <span className="truncate">{project.category}</span>
            </div>
            <h3 className="mt-1 font-display text-base md:text-lg text-white truncate">
              {project.title}
            </h3>
          </div>
          <div className="flex items-center gap-1">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="ml-2 inline-flex items-center gap-1.5 px-3 py-1.5 text-[10px] uppercase tracking-[0.3em] text-white/70 hover:text-[hsl(0_72%_55%)] transition-colors"
            >
            ...
            <button
              ...
              className="ml-1 p-2 rounded text-white/50 hover:text-white hover:bg-white/5 transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        {/* Footer: outcomes */}
        <footer className="border-t border-white/[0.06] px-6 py-5 md:px-8 grid gap-4 md:grid-cols-[2fr_1fr]">
        ...
        {isExternal && (
          <p className="px-6 md:px-8 pb-3 text-[10px] uppercase tracking-[0.3em] text-white/30">
...
function FallbackPreview({ project }: { project: Project }) {
  return (
    <div className="flex h-full min-h-[400px] w-full items-center justify-center bg-[#0a0a0a] p-10">
      <div className="max-w-md text-center">
        <p className="text-[10px] uppercase tracking-[0.4em] text-[hsl(0_72%_55%)]">
          Embedded preview unavailable
        </p>
        <h4 className="mt-4 font-display text-xl text-white">{project.title}</h4>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="mt-6 inline-flex items-center gap-2 border border-white/15 px-5 py-3 text-[11px] uppercase tracking-[0.35em] text-white/80 hover:border-[hsl(0_72%_55%)] hover:text-[hsl(0_72%_55%)] transition-colors"
        >
